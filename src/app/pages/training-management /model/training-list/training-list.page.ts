...
// import { TrainingDetailPageModel } from '../training-details/training-detail.page';
...
export class TrainingListPageModel implements OnInit {
  ...
  deleteTraining(noteId){
    let responseMsg:String;
    this._noteManagementService.deleteTraining(noteId).subscribe(async resp => {
      if(!resp.error){
        this.getFolderNotes(this.folderId);
      }
  ...
  async openNoteDetailModal(data:any) {
    const modal = await this.modalController.create({
      component: TrainingDetailPageModel,
      componentProps:{
        note : data,
