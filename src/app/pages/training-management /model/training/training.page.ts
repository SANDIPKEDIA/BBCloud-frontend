...
export class TrainingPageModel implements OnInit {
  ...
  updateTraining(payload) {
    ...
    this._noteManagementService.updateTraining(id, formData).subscribe(async (resp) => {
  ...
  createTraining(payload: FormGroup) {
    let formData = JSON.parse(JSON.stringify(payload.value));
    formData["userId"] = localStorage.getItem("adminId") || "601870f796b9f2834f045d1a"
    formData["date"] = this.dateFormater(formData.date); 
    console.log("create payload",formData);
    this._noteManagementService.createTraining(formData).subscribe(async (resp) => {
      this.responseStr = resp.response;
      let toast = await this.toast.create({
